...
// カラーパレット（インデックス → CSS色、16進数インデックス 0-f）
const PALETTE = [
  null,        // 0: 透明
  '#e8dfc8',   // 1: クリーム（オリジナル・ネコ・うさぎ体）
  '#180f00',   // 2: アウトライン
  '#180f00',   // 3: 目
  '#c8283c',   // 4: 口・赤
  '#ffb8cc',   // 5: ほっぺ・ピンク
  '#ffffff',   // 6: ハイライト・白
  '#50c030',   // 7: スライムグリーン
  '#a0e870',   // 8: スライムライトグリーン
  '#f8c030',   // 9: 鳥イエロー
  '#e8c0f8',   // a(10): うさぎライトパープル
  '#f07820',   // b(11): 鳥くちばし・オレンジ
  '#9060e0',   // c(12): モンスターパープル
  '#ff2040',   // d(13): モンスター赤目
  '#60d890',   // e(14): モンスターグリーン（予備）
  '#c8c8d8',   // f(15): グレー（予備）
];
...
function getSpriteName() {
  const { stage, variant, petType } = state;
  const type = petType || 'original';
  ...
  if (type === 'original') {
    ...
    if (stage === 'child') return `${type}_child`;
    if (stage === 'young') return `${type}_young_${variant}`;
    if (stage === 'adult') return `${type}_adult_${variant}`;
  }
  return 'egg';
}
...
const PET_TYPES = ['original', 'cat', 'slime', 'monster', 'rabbit', 'bird'];
...
function evolveToStage(newStage) {
  // たまご→こども のとき、ランダムにペット種を決定
  if (state.stage === 'egg' && newStage === 'child') {
    state.petType = PET_TYPES[Math.floor(Math.random() * PET_TYPES.length)];
  }
  ...
  let evoMsg;
  if (newStage === 'child') {
    const petName = PET_TYPE_NAMES[state.petType] || '';
    evoMsg = `${petName} の こが うまれたよ！\nなかよく してあげてね！`;
  } else if (newStage === 'adult') {
    evoMsg = `${variantLabels[state.variant]} に なったよ！\nいっぱい あそんでね！`;
  } else {
    evoMsg = 'すくすく そだっているよ！\nこれからも よろしくね！';
  }
  document.getElementById('evo-message').textContent = evoMsg;
...
/** 選択画面でペット種別を決定してゲーム開始 */
function selectPet(type) {
  state.petType = type;
  startMainGame();
}

/** 選択画面のCanvasにプレビュースプライトを描画 */
function renderSelectScreen() {
  const previews = {
    original: 'child',
    cat:      'cat_child',
    slime:    'slime_child',
    monster:  'monster_child',
    rabbit:   'rabbit_child',
    bird:     'bird_child',
  };
  const ps = 4; // 選択画面は小さめ
  Object.entries(previews).forEach(([type, spriteName]) => {
